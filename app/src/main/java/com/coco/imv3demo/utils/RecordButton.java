        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (tempNowY - startY < CANCLE_LENGTH) {
                    mTitleTv.setText(getContext().getString(R.string.cancel));
                    if (finishedListener != null) {
                        finishedListener.readCancel();
                    }
                } else {
                    mTitleTv.setText(getContext().getString(R.string.cancel2));
                    if (finishedListener != null) {
    ...
    private void finishRecord() {
        ...
        if (intervalTime < MIN_INTERVAL_TIME) {
            if (TextUtils.isEmpty(tooShortToastMessage)) {
                Toast.makeText(getContext(), getContext().getResources().getString(R.string.tooShort), Toast.LENGTH_SHORT).show();
            } else {
